        ...
        <div class="col-12">
            <div class="card">
                ...
                <DataTable
                    ref="dt"
                    :value="functions"
                    v-model:selection="selectedFunctions"
                    dataKey="id"
                    :paginator="true"
                    :rows="10"
                    :filters="filters"
                ...
                >
                    ...
                    <Column headerStyle="min-width:10rem;">
                        <template #body="slotProps">
                            ...
                            <Button icon="pi pi-trash" class="mt-2" severity="warning" rounded @click="confirmDeleteFunction(slotProps.data)" />
                        </template>
                    </Column>
                </DataTable>

                <Dialog v-model:visible="deleteFunctionDialog" :breakpoints="{ '960px': '75vw' }" style="width: 30vw" header="Confirm" :modal="true">
                    <div class="flex align-items-center justify-content-center">
                        <i class="pi pi-exclamation-triangle mr-3" style="font-size: 2rem" />
                        <span v-if="functionss"
                            >Are you sure you want to delete <b>{{ functionss.name }}</b
                            >?</span
                        >
                    </div>
                    <template #footer>
                        <Button label="No" icon="pi pi-times" text @click="deleteFunctionDialog = false" />
                        <Button label="Yes" icon="pi pi-check" text @click="deleteFunction" />
                    </template>
                </Dialog>
                <Dialog v-model:visible="addModal" :modal="true" header="Add Function" :style="{ width: '50vw' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
...
const functions = ref([]);
const deleteFunctionDialog = ref(false);
const functionss = ref({});
...
const confirmDeleteFunction = (editFunction) => {
    functionss.value = editFunction;
    deleteFunctionDialog.value = true;
};
...
const openEditModal = (editFunction) => {
    isEditing.value = true;
    forms.id = editFunction.id;
    forms.name = editFunction.name;
    forms.detail = editFunction.detail;
    forms.status = editFunction.status;
    addModal.value = true;
};
...
const handleEditSubmit = async () => {
    try {
        ...
        const response = await functionService.updateFunction(formData);
        if (response.status) {
...
const handleSubmit = async () => {
    try {
        ...
        const response = await functionService.createFunction(formData);
        if (response.status) {
            toast.add({ severity: 'success', summary: 'Success', detail: 'Function added successfully', life: 3000 });
            addModal.value = false;
            resetForm();
            const data = await functionService.getFunctions();
            functions.value = data.data;
...
const deleteFunction = async () => {
    loading.value = true;
    try {
        await functionService.deleteFunction(functionss.value.id);
        functions.value = functions.value.filter((val) => val.id !== functionss.value.id);
        deleteFunctionDialog.value = false;
        functionss.value = {};
